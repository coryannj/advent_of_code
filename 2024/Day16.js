...
console.log(Math.floor(Math.log10(Math.abs(1410))))
console.log(Math.pow(10,Math.floor(Math.log10(Math.abs(1410)))))
...
console.log([...Object.keys(grid)].slice(1,-1).map((x)=>grid[x]))

console.log([...grid.gridMap.keys()].at(-1))

//const coord = (mapInd) => [(mapInd-(mapInd%grid.rowFactor))/grid.rowFactor,mapInd%grid.rowFactor]
...
const nextStep = (mapInd,filterFn) => {
    let next = [
        ...
        [grid.colFactor,"E"],
        [-grid.colFactor,"W"]
    ].map(([ind,dir])=>[mapInd+ind,dir]).filter(([ind,dir])=>grid.map.get(ind))
...
let deadends = [...grid.gridMap.entries()].filter(([i,v])=> v !== '#').filter((x)=>nextStep(x[0],onPath).length<2)

while(deadends.length>0){
    for(const[i,v] of deadends){
        grid.gridMap.set(i,"#")
        grid.gridArr[Math.floor(i/grid.rowFactor)][i%grid.rowFactor]="D"
    }

    deadends = [...grid.gridMap.entries()].filter(([i,v])=> v !== '#').filter((x)=>nextStep(x[0],onPath).length<2)
}
//start from here

let queue = nextStep(startInd,onPath).map(([ind,dir])=>[[startInd,dir],[ind,dir]])
let paths = []
visited = queue.flat().map((x)=>x[0])

while(queue.length>0){
    let thisPath = queue.shift()
    let lastStep = [lastInd,lastdir] = thisPath.at(-1)
    //visited.push(lastInd)
    let nextSteps = nextStep(lastInd,onPath)
    let sameDir = (nextSteps.length === 0 || !nextSteps.map((x)=>x[1]).includes(lastdir) || nextSteps.length>1)
    ...
    for(const[i,d] of nextSteps){
        if(i === endInd){
            paths.push([...thisPath,[i,d]])
            console.log([...thisPath,[i,d]])
        } else {
            visited.push(i)
            queue.push([...thisPath,[i,d]])
        }
    }
}
