...
// Gets tilt for either 'north' or 'west'
...
// Cycles through all four tilts, saves array/load/seenindex 
function cycles(number) {
  ...
  let aftercycle = mirrors2.map((x)=> x.join('')) // Make string to add to seen arrays
 
  // If we have seen array - add index + load value to sep arrays so we can identify cycle length (you could also just math it)
  if (mirrorSet.includes(aftercycle.join('')) === true) {
    ...
    if (foundindex.length === 0) {
      firstseen = cyclecounter // sets value for first repeated array
    }

    if (foundindex.includes(foundind)) {
      //console.log('we found the cycle!!!!!',foundind,foundload,' already in indexes')
      break;
    } else {
