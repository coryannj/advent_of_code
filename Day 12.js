...
function getArrangements (spr,br,p1orp2) {
  const firstLastRegex = new RegExp(/(^[#]|[#]$)/,'m')
  ...
  if (wriggle === 0 || springsSplit.join('').split('.').length === brokenMap.join('').split('.').length && springsSplit.join('').split('.').every((x)=>  x.includes('#') && firstLastRegex.test(x))) {
    return 1
  } else {
    // Set starting states
    statesCount.set(0,1)
          ...
          if (stateIdx<endStateIdx) {
            if ((nextState === '#' && spring !== '.'|| nextState === '.' && spring !== '#') && (stateIdx<secondLastStateIdx-1||!springsSplit.join('').substring(spridx+1).includes('#'))) {
              ...
              if (nextState === '.' && statesCount.has(stateIdx+1)) { 
                nextStateVal=statesCount.get(stateIdx+1); // Get prev count to add to for '.' states
              }
