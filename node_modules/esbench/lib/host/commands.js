...
import JobGenerator from "./toolchain.js";
import { messageResolver } from "../connect.js";
import { HostContext } from "./context.js";
function loadResults(path, throwIfMissing) {
    try {
        return JSON.parse(readFileSync(path, "utf8"));
    }
    catch (e) {
        if (throwIfMissing || e.code !== "ENOENT")
            throw e;
    }
}
export async function report(config, patterns) {
...
export async function start(config, filter) {
...
async function runJob(context, job, writer) {
