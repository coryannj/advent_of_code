 ...
 */
export function minimalLeastSquare(input, time, fn) {
 ...
 */
export function welchTest(a, b, alt) {
    ...
    const df = (se ** 2) / ((seA ** 2) / (a.length - 1) +
        (seB ** 2) / (b.length - 1));
...
function studentOneTail(t, df) {
    return t < 0 ? 1 - studentTwoTail(t, df) / 2 : studentTwoTail(-t, df) / 2;
}
// https://www.math.ucla.edu/~tom/distributions/tDist.html
function studentTwoTail(t, df) {
    const a = df / 2;
    const s = a + 0.5;
    const z = df / (df + t * t);
    const bt = Math.exp(logGamma(s) - logGamma(0.5) - logGamma(a)
